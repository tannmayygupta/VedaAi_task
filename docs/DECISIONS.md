...
## [2026-08-26] Resolved: PDF answer sheets now render with real highlight overlays via pdfjs-dist

**Decision:** Added `pdfjs-dist` to actually render PDF answer-sheet pages to a `<canvas>`
(`src/lib/pdf/pdfjs.ts`, `src/lib/mapping/usePdfDocument.ts`,
`src/components/mapping/PdfPageCanvas.tsx`), replacing the old "preview isn't available yet" text
placeholder in `AnswerSheetViewer.tsx`. The existing percentage-based `AnswerHighlight` overlay
now draws on top of the rendered canvas exactly as it already did for raw image answer sheets —
no changes needed there, confirming the earlier design choice (percentage coordinates, not
pixel-absolute) paid off. Real page counts now come from pdfjs (`pdf.numPages`), replacing the
old hardcoded "1 page for any PDF" in `answerSheetPageSource.ts`, whose PDF-handling branch is
now dead code (removed) since `AnswerSheetViewer` branches on PDF-vs-image itself before
delegating to it. The same pdfjs-dist dependency also replaced the upload screen's page-count
heuristic (`src/lib/validation/pageCount.ts`) — a regex counting `/Type /Page` markers in raw
PDF bytes — with a real page count, since the dependency was being added anyway.

**Why:** This closes the visual side of the multi-image-merge fix above: once multi-image answer
sheets became single merged PDFs, they lost even the partial image preview they used to have,
inheriting the pre-existing "no PDF preview" limitation instead. Researched pdfjs-dist's current
Next.js/Turbopack integration before implementing (see chat) — confirmed a known, documented
Turbopack bug in pdfjs-dist's default worker-loading path
([vercel/next.js#65406](https://github.com/vercel/next.js/issues/65406)), worked around by
copying the worker file into `public/` at install time
(`scripts/copy-pdf-worker.mjs`, wired to `postinstall`) and pointing `GlobalWorkerOptions.workerSrc`
at that static path instead of letting pdfjs-dist dynamically resolve it. Zoom (confirmed via the
connected Figma file to be a real part of the design, not an assumption — node `1:9076` in the
shared file shows the exact "−/100%/+" + "‹ Page X of N ›" header already built) is handled by
rendering each page once at a fixed higher resolution (`PDF_RENDER_SCALE = 2`) and letting CSS
scale the wrapper, matching how the image path already handles zoom, rather than re-rendering the
PDF on every zoom click — simpler and consistent, at the cost of the same blur-at-high-zoom
trade-off images already accept.

**Alternatives considered:** `react-pdf` (a React wrapper around pdfjs-dist) — rejected; it adds a
dependency layer with its own overlapping Turbopack/worker GitHub issues without removing the one
real problem, and its `<Document>`/`<Page>`/text-layer machinery solves problems this feature
doesn't have (we only need canvas pixels for a coordinate overlay, not text selection or
annotations). Server-side PDF-to-image conversion at merge/upload time — rejected; native
rasterization tools (Poppler/Ghostscript) don't inline cleanly into Vercel serverless functions,
while client-side rendering has no such constraint. Re-rendering the PDF page at a new scale on
every zoom step (the textbook-correct pattern for general-purpose PDF viewers) — rejected in favor
of the simpler fixed-resolution-plus-CSS-scale approach, since this viewer's zoom range is narrow
(50%-200%, per the existing `ZoomControl`) and the image path already accepts the same trade-off.

**Trade-offs / risks:** Getting a PDF's page count now requires actually opening the file
(pdfjs-dist), which is asynchronous — replacing what used to be instant (a text-heuristic guess).
This shows briefly as a loading state where there wasn't one before; expected, not a regression.
`pdf.worker.min.mjs` is gitignored and regenerated by `postinstall` (copied from
`node_modules/pdfjs-dist`) rather than committed, so it must never be manually deleted from
`public/` without re-running `npm install`. jsdom has no `DOMMatrix`, which pdfjs-dist constructs
at module-load time even for code paths that never touch canvas rendering — added a minimal
test-only polyfill in `vitest.setup.ts` so importing pdfjs-dist anywhere in a test (even
transitively, e.g. via the upload screen's page-count chip) doesn't crash; this stub is
intentionally not a correct `DOMMatrix` implementation since real rendering is verified in a real
browser (claude-in-chrome), never in jsdom. Verified end-to-end against real Gemini output on the
same merged 2-page fixture from the multi-image-merge fix above: the highlight badge for Q1
(page 1) and Q7 (page 2, cross-page navigation) both rendered in the correct position on the
correct page, and stayed aligned with the canvas after zooming to 130%.

**Process note:** Live verification was blocked mid-session by Gemini's free-tier **daily** quota
(`GenerateRequestsPerDayPerProjectPerModel-FreeTier`, limit 20/day) being exhausted from
cumulative real-API testing earlier today — not a per-minute cooldown despite the misleading
"retry in 46s" text in Gemini's own error response. The user provided a replacement
`GEMINI_API_KEY` (value not logged anywhere) to continue verification the same day; `.env.local`
was updated directly, never printed to chat. Worth remembering for future sessions: this project's
